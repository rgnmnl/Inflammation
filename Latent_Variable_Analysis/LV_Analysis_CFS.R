...
varlist <- grep("(^crp($|_1)|^il6($|_1)|^il8($|_1)|^il10($|_1)|^il18($|_1)|^icam($|_1)|^tnfa($|_1)|^pselectin($|_1)|^eselectin($|_1)|^l1_beta($|_1)|^tnfa_r1($|_1)|^mmp1($|_1)|^mmp9($|_1)|^cd40($|_1)|^isoprostane_8_epi_pgf2a($|_1)|^lppla2_act($|_1)|^lppla2_mass($|_1)|^mcp1($|_1)|^mpo($|_1)|^opg($|_1)|^tnfr2($|_1))", 
                colnames(CFS), value = TRUE, ignore.case = TRUE)
...
assign(paste("CFS", "trans", sep = "_"), trans.df)

for(j in 1:length(varlist)){
  # varcol <- get(study[i])[[varlist[j]]] %>% as.numeric()
  if(sum(is.na(CFS_trans[, varlist[j]])) == nrow(CFS_trans)){
    varlist[j] <- NA
  }     
}
varlist <- varlist[!is.na(varlist)]

summary(CFS_trans[, varlist])

mod <- "comp.pheno =~ IL6 + CRP + ICAM + TNFA"
fit <- cfa(mod, data=CFS_trans,missing = "ml")
...
CFS_lv <- tibble::rownames_to_column(trans.df, "id") %>% mutate(id = as.numeric(id)) %>% left_join(., pred, by = "id") %>% dplyr::select(-1)

CFS_encore_prelim <- left_join(CFS_lv[, c("sample.id", "comp.pheno", "SEX", "ANCESTRY")], pcs, by = c("sample.id"="V1"))
names(CFS_encore_prelim)[5:15] <- c("PC1", "PC2", "PC3", "PC4", "PC5", "PC6", "PC7", "PC8", "PC9", "PC10", "PC11")
write.table(CFS_encore_prelim, "../Inflammation_SEM/CFS_encore_prelim.txt", sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
