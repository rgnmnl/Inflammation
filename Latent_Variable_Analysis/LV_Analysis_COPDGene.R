...
# ***
# mod <- "comp.pheno =~ IL6 + CRP + IL8 + IL10 + ICAM + ESELECTIN + TNFA + TNFA_R1" 
# fit <- cfa(mod, data=COPDGene_trans, missing = "ml")
# summary(fit, standardized = T, fit.measures = T)
# 
# mod <- "comp.pheno =~ IL6 + CRP + IL8 + IL10 + ICAM + ESELECTIN + TNFA"
# fit <- cfa(mod, data=COPDGene_trans, missing = "ml")
# summary(fit, standardized = T, fit.measures = T)
# 
# mod <- "comp.pheno =~ IL6 + CRP + IL8 + IL10 + ICAM + TNFA"
# fit <- cfa(mod, data=COPDGene_trans, missing = "ml")
# summary(fit, standardized = T, fit.measures = T)
# 
# mod <- "comp.pheno =~ IL6 + IL10 + TNFA"
# fit <- cfa(mod, data=COPDGene_trans, missing = "ml")
# summary(fit, standardized = T, fit.measures = T)
# 
...
names(COPDGene_PCs)[7:17] <- c("PC1", "PC2", "PC3", "PC4", "PC5", "PC6", "PC7", "PC8", "PC9", "PC10", "PC11")

write.table(COPDGene_PCs[!is.na(COPDGene_PCs$comp.pheno),], "../Inflammation_SEM/COPDGene_encore_prelim.txt", sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
